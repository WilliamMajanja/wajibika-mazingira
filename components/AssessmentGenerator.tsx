...
export const AssessmentGenerator: React.FC = () => {
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    if (!process.env.API_KEY) {
        addToast({ type: 'error', message: 'API_KEY is not configured. Cannot generate assessment.' });
        return;
    }
    ...
    try {
      ...
      if (report.includes("An error occurred")) {
         addToast({ type: 'error', message: report });
         setGeneratedReport(null);
      } else {
